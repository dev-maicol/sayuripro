...
@section('content')
    <div class="max-w-md mx-auto mt-10 bg-gray-800 p-6 rounded-lg shadow-md">
        <h2 class="text-xl font-semibold text-white mb-6">Crear cargo</h2>

        <form action="{{ route('cargos.store') }}" method="POST" x-data="{ loading: false }" @submit="loading = true">
            @csrf
            ...
            <div class="mb-6">
                ...
                <textarea name="funciones" id="funciones" rows="4"
                    class="mt-1 block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('funciones') }}</textarea>
                @error('funciones')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
