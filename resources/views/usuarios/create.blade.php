...
@section('content')
    <div class="p-6 bg-white dark:bg-gray-800 rounded-lg shadow max-w-xl mx-auto">
        <h2 class="text-xl font-bold mb-4 text-white">Crear Usuario</h2>
        ...
        <form method="POST" action="{{ route('usuarios.store') }}" x-data="{ loading: false }"
            @submit.prevent="loading = true; $el.submit()">
            @csrf

            <div class="mb-4">
                <label class="block text-white" for="name">Nombre</label>
                <input type="text" name="name" class="w-full p-2 rounded" value="{{ old('name') }}">
            </div>

            <div class="mb-4">
                <label class="block text-white">Correo</label>
                <input type="email" name="email" class="w-full p-2 rounded" value="{{ old('email') }}">
            </div>

            <div class="mb-4">
                <label class="block text-white">Contraseña</label>
                <input type="password" name="password" class="w-full p-2 rounded">
            </div>

            <div class="mb-4">
                <label class="block text-white">Confirmar Contraseña</label>
                <input type="password" name="password_confirmation" class="w-full p-2 rounded">
            </div>
            ...
            <a href="{{ route('usuarios.index') }}" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">
                Cancelar
            </a> --}}
